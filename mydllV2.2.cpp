...
int propotion_long[NUM_KINDS_SOLDIER] = { 0,0,1,2,0,2,2,2 };    //后期造兵造兵比例
...
public:
	...
	void march();
	void attack();    //攻击当前攻击塔
	void attack(int soldierid);   //攻击某士兵
...
void Troop::attackplace(int x, int y) {
	...
	for (int i = 0; i < 2 * length + 1; i++) {
		for (int j = 0; j < 2 * length + 1; j++) {
			...
			if (distance(base.x_position, base.y_position, score[rt][0], score[rt][1]) <= base.move_left) score[rt][2] -= 5;
			if (distance(base.x_position, base.y_position, score[rt][0], score[rt][1]) <= length) score[rt][2] -= 5;
...
void Troop::search(int type) {
	int a = base.x_position+(2*way[0]-5)*delta[way[0]];
	int b = base.y_position+(1-2*delta[2])*delta[way[2]];
	length = move_left;
	int min = move_left, c = a -length, d = b -length;
	place[0] = a; place[1] = b;
	for (int i = 0; i < length; i++) {
		for (int j = 0; j < length; j++) {
			if (flag(c + i, d + j)) {
...
void Troop::attack(int soldierid) {
